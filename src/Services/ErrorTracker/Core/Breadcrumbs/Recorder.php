...
class Recorder implements Countable, Iterator
{
     ...
     *
     * @var \TechiesAfrica\Devpilot\Services\ErrorTracker\Core\Breadcrumbs\Breadcrumb[]
     */
    private $breadcrumbs = [];
     ...
     * Add a breadcrumb.
     *
     * @param \TechiesAfrica\Devpilot\Services\ErrorTracker\Core\Breadcrumbs\Breadcrumb $breadcrumb
     *
     * @return void
     */
    ...
    public function setMaxBreadcrumbs($maxBreadcrumbs)
    {
        if (!is_int($maxBreadcrumbs) || $maxBreadcrumbs < 0 || $maxBreadcrumbs > 100) {
            error_log(
                'Devpilot Warning: maxBreadcrumbs should be an integer between 0 and 100 (inclusive)'
            );
